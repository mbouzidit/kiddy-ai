...
- **Translations always in pairs** — every new user-facing string must have both an `en` and `fr` entry in `data.js`. Never hardcode display strings directly in JS or HTML.
- **`t()` for all UI text** — use the `t('key')` helper for any string shown to the user.
